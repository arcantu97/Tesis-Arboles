...
Esta fase es la más importante de todas debido a que se utiliza el modelo generado a partir de la fase de entrenamiento. En esta fase se utilizan las características globales de extracción de características de la sección 2.3 donde se hace uso del modelo clasificador de {\em bosque aleatorio} (inglés: {\em random forest})\footnotemark, donde se establece un valor estimado de arboles por cada muestra donde se vaya a probar el modelo, en la investigación se va a utilizar un valor de 100. Posteriormente se tiene que definir que utilizar la información de los modelos de características utilizadas y las etiquetas de muestras generadas a partir de ello también. 
...
Después de clasificar todas las muestras que pasaron por las fases de entrenamiento, detección y combinación, se puede obtener un resultado preliminar que indica cuantas especies fueron detectadas en la fase de detección.
...
%anterior, se dejó un límite de 0.05\% de píxeles ausentes y para comparar los valores límite entre dos experimentos, se realiza el experimento 
\section{Resultados}
...
Luego de probar el modelo generado en el capítulo 5.1, el cual se genero a partir de las muestras con suelo removido, se pudo obtener como resultado la cantidad de muestras generadas.
