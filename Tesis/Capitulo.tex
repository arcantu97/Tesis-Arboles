...
En la figura 4.3 y 4.4 se muestra un ejemplo de cómo se verían las muestras que son de utilidad y destacando que el siguiente capítulo define el procedimiento realizado para poder obtener muestras útiles.
...
\begin{figure}[h!]
  ...
  \begin{minipage}[b]{1\textwidth}
        \includegraphics[width=\textwidth]{result_umbralización}
...
\begin{figure}[h!]
  \centering  
  \begin{minipage}[b]{1\textwidth}
        \includegraphics[width=\textwidth]{resultado_pixelesausentes}
...
La solución desarrollada se pensó en realizar con el aprendizaje máquina y la visión computacional como parte de una solución eficiente que pueda ahorrar tiempos y costos a las personas expertas del área en que se va a aplicar la solución. Los métodos tradicionales son bastante costos y tardados por la complejidad de recolectar muestras, además de lo que implica viajar con un equipo eficiente a las zonas forestales, por tal, la solución propone reducir tiempos y automatizar tareas como el procesamiento y la clasificación de muestras para hacer más eficiente el generar un inventario un forestal a partir de la visión computacional. 
\clearpage
Esta solución además, está desarrollada con el lenguaje Python en su versión 3, la cual no requiere de muchos recursos para ejecutarse, sin embargo, la ejecución y el tiempo de resultados dependerá en gran medida del equipo en que se ejecute. Esto último puede que no sea una limitante debido a que las computadoras actuales ya son bastante eficientes para ejecutar un script de Python en segundo plano, sin embargo, si es un factor a considerar en cuanto al tiempo de procesamiento para garantizar que esta pueda ser ejecutada sin muchos inconvenientes. 

En cuanto a la parte técnica de la solución, ésta se apoya de bibliotecas como Scikit, OpenCV, Scipy y Pillow, mismas que están documentadas por versión en el repositorio donde se alojan los scripts utilizados para la generación del inventario forestal y la tesis escrita \citep{rf18}.
...
En general, también podrían reducirse algunos procesos e ir recopilando cada el resultado de cada fase para aumentar la productividad del algoritmo e ir generando secuencialmente la solución que se vaya a implementars.
